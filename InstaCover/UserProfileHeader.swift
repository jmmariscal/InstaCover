...
//  InstaCover
//
//  Created by Juan M Mariscal on 4/11/17.
...
class UserProfileHeader: UICollectionViewCell {
    ...
    fileprivate func setupEditFollowButton() {
        guard let currentLoddedInUserId = FIRAuth.auth()?.currentUser?.uid else { return }
        
        guard let userId = user?.uid else { return }
        
        if currentLoddedInUserId == userId {
            //edit profile
        } else {
            
            //check if 'Following'
            FIRDatabase.database().reference().child("following").child(currentLoddedInUserId).child(userId).observeSingleEvent(of: .value, with: { (snapshot) in
...
//            
//            //need to get back on to Main UI Thread
